...
mod full {
    ...
    pub fn apply(domains: &[String]) -> Result<usize, String> {
        ...
        if !domains.is_empty() {
            ...
            for d in domains {
                let d = d.trim();
                // Defense in depth: patterns are validated on input, but never
                // write a value here that isn't a bare token - a stray space,
                // newline, or '#' could otherwise inject arbitrary hosts entries
                // under elevation.
                if d.is_empty()
                    || d.bytes()
    ...
    fn write_hosts(content: String) -> Result<(), String> {
        ...
        #[cfg(not(target_os = "linux"))]
        {
            ...
            // ENTIRE hosts file truncated/empty if the process is killed or power
            // is lost mid-write. `std::fs::rename` replaces atomically within a
            // volume on both Windows and macOS.
            let tmp = path.with_extension("systemtrace-tmp");
            {
                let mut f = fs::File::create(&tmp).map_err(|e| {
                    format!("could not write hosts file (run as administrator): {e}")
                })?;
